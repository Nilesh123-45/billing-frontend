...
const CartSummary = ({customerName,mobileNumber,setMobileNumber,setCustomerName}) => {
    ...
    const completePayment = async (paymentMode) =>{
        ...
        toast.error("Please enter valid details of the customer");
        return;
    }
        ...
        try{
             ...
             if(response.status ==201 && paymentMode == "cash"){
                ...
                if(!razorpayLoaded){
                    ...
                    await deleteOrderOnFailure(savedData.orderId);// this makes sure that if the razorpaypayment is unsuccessful then the saved order aytomatically deleted
                    return;
                }
               ...
               rzp.on("payment.failed",async(response)=>{
                 await deleteOrderOnFailure(savedData.orderId);
                toast.error("Payment failed buddy");
    ...
    return(
        <div className="mt-2">
            ...
            <div className="d-flex gap-3">
                ...
                >
                    {isProcessing ? "Proccessing the payment...":"Cash"}
                </button>
                <button className="btn btn-primary flex-grow-1"
                onClick={()=>completePayment("upi")}
                disabled={isProcessing}
                >
                    {isProcessing ? "Proccessing the payment...":"UPI"}
                </button>
            </div>
